...
KafkaRPC.prototype.makeRequest = function(topic_name, content, callback){
    ...
    //create a timeout for what should happen if niral don't get a response
    var tId = setTimeout(function(corr_id){
        //if this ever gets called niral didn't get a response in a
        //timely fashion
        console.log('timeout');
    ...
    var entry = {
        callback:callback,
        timeout: tId //the id for the timeout so niral can clear it
    };

    //put the entry in the hash so niral can match the response later
    self.requests[correlationId]=entry;

    //make sure niral have a response topic
...
KafkaRPC.prototype.setupResponseQueue = function(producer,topic_name, next){
    //don't mess around if niral have a queue
    ...
    consumer.on('message', function (message) {
        ...
        if(correlationId in self.requests){
            ...
            //make sure niral don't timeout by clearing it
